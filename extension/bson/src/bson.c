...
Datum bson_out(PG_FUNCTION_ARGS)
{
    ...
    if (!bson_init_static(&bson_doc, (const uint8_t*)VARDATA_ANY(arg),
        ...
        ereport(ERROR,
                (errcode(ERRCODE_DATA_CORRUPTED), errmsg("invalid BSON data")));
 ...
 */
Datum bson_cmp(PG_FUNCTION_ARGS)
{
 ...
 */
Datum bson_eq(PG_FUNCTION_ARGS)
{
    bytea* arg1 = PG_GETARG_BYTEA_PP(0);
    bytea* arg2 = PG_GETARG_BYTEA_PP(1);
    int len1, len2;
 ...
 */
Datum bson_lt(PG_FUNCTION_ARGS)
{
    Datum cmp_result =
        DirectFunctionCall2(bson_cmp, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));

    PG_RETURN_BOOL(DatumGetInt32(cmp_result) < 0);
}

Datum bson_le(PG_FUNCTION_ARGS)
{
    Datum cmp_result =
        DirectFunctionCall2(bson_cmp, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
...
Datum bson_gt(PG_FUNCTION_ARGS)
{
    Datum cmp_result =
        DirectFunctionCall2(bson_cmp, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
...
Datum bson_ge(PG_FUNCTION_ARGS)
{
    Datum cmp_result =
        DirectFunctionCall2(bson_cmp, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
...
Datum bson_ne(PG_FUNCTION_ARGS)
{
    Datum eq_result =
        DirectFunctionCall2(bson_eq, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
...
Datum bson_hash(PG_FUNCTION_ARGS)
{
    ...
    result = hash_any((unsigned char*)VARDATA_ANY(arg), VARSIZE_ANY_EXHDR(arg));
...
Datum bson_exists(PG_FUNCTION_ARGS)
{
    bytea* bson_data = PG_GETARG_BYTEA_PP(0);
    text* path_text = PG_GETARG_TEXT_PP(1);
    bson_t bson;
    ...
    if (!bson_init_static(&bson, (const uint8_t*)VARDATA_ANY(bson_data),
                          VARSIZE_ANY_EXHDR(bson_data)))
    {
        ereport(ERROR,
                (errcode(ERRCODE_DATA_CORRUPTED), errmsg("invalid BSON data")));
    }
...
Datum bson_exists_any(PG_FUNCTION_ARGS)
{
    bytea* bson_data = PG_GETARG_BYTEA_PP(0);
...
Datum bson_exists_all(PG_FUNCTION_ARGS)
{
    bytea* bson_data = PG_GETARG_BYTEA_PP(0);
    ArrayType* paths_array = PG_GETARG_ARRAYTYPE_P(1);
    bson_t bson;
...
Datum bson_contains(PG_FUNCTION_ARGS)
{
    ...
    if (!bson_init_static(&bson1, (const uint8_t*)VARDATA_ANY(bson1_data),
                          VARSIZE_ANY_EXHDR(bson1_data)))
    {
        ereport(ERROR, (errcode(ERRCODE_DATA_CORRUPTED),
                        errmsg("invalid BSON data in first argument")));
    }

    if (!bson_init_static(&bson2, (const uint8_t*)VARDATA_ANY(bson2_data),
                          VARSIZE_ANY_EXHDR(bson2_data)))
    {
        ereport(ERROR, (errcode(ERRCODE_DATA_CORRUPTED),
                        errmsg("invalid BSON data in second argument")));
    }
            ...
            if (bson_iter_init_find(&iter1, &bson1, key))
            {
                ...
                else
                {
                    ...
                    case BSON_TYPE_INT64:
                        ...
                        break;
                    case BSON_TYPE_DOUBLE:
                        if (bson_iter_double(&iter1) !=
                            bson_iter_double(&iter2))
                            result = false;
                        break;
                    case BSON_TYPE_BOOL:
...
Datum bson_contained(PG_FUNCTION_ARGS)
{
    /* bson_contained(A, B) is equivalent to bson_contains(B, A) */
    return DirectFunctionCall2(bson_contains, PG_GETARG_DATUM(1),
                               PG_GETARG_DATUM(0));
}
