 ...
 */
static BsonData *
bson_t_to_bson_data(const bson_t *bson_doc)
{
	BsonData   *result;
	const uint8_t *data;
	uint32_t	len;
...
Datum
bson_array_agg_transfn(PG_FUNCTION_ARGS)
{
	MemoryContext old_context;
	MemoryContext agg_context;
	BsonAggState *state;
...
Datum
bson_array_agg_finalfn(PG_FUNCTION_ARGS)
{
	BsonAggState *state;
	BsonData   *result;
...
Datum
bson_extract_field(PG_FUNCTION_ARGS)
{
...
bson_merge(PG_FUNCTION_ARGS)
{
	...
	if (!bson_init_static(&bson1, (const uint8_t *)VARDATA_ANY(arg1), VARSIZE_ANY_EXHDR(arg1)))
	{
		ereport(ERROR,
	...
	if (!bson_init_static(&bson2, (const uint8_t *)VARDATA_ANY(arg2), VARSIZE_ANY_EXHDR(arg2)))
	{
		ereport(ERROR,
				(errcode(ERRCODE_DATA_CORRUPTED),
		...
		while (bson_iter_next(&iter))
		{
			...
			if (found)
			{
				...
				while (bson_iter_next(&result_iter))
				{
					...
					{
						const bson_value_t *existing_value = bson_iter_value(&result_iter);
						bson_append_value(&temp_result, existing_key, -1, existing_value);
					}
				}
...
Datum
bson_count_field(PG_FUNCTION_ARGS)
{
	BsonData   *bson_data = (BsonData *) PG_GETARG_POINTER(0);
	text	   *field = PG_GETARG_TEXT_P(1);
	char	   *field_str = text_to_cstring(field);
...
Datum
bson_sum_field(PG_FUNCTION_ARGS)
{
	BsonData   *bson_data = (BsonData *) PG_GETARG_POINTER(0);
	text	   *field = PG_GETARG_TEXT_P(1);
	char	   *field_str = text_to_cstring(field);
...
Datum
bson_avg_field(PG_FUNCTION_ARGS)
{
	BsonData   *bson_data = (BsonData *) PG_GETARG_POINTER(0);
	text	   *field = PG_GETARG_TEXT_P(1);
	char	   *field_str = text_to_cstring(field);
...
Datum
bson_min_field(PG_FUNCTION_ARGS)
{
	BsonData   *bson_data = (BsonData *) PG_GETARG_POINTER(0);
	text	   *field = PG_GETARG_TEXT_P(1);
	char	   *field_str = text_to_cstring(field);
...
Datum
bson_max_field(PG_FUNCTION_ARGS)
{
	BsonData   *bson_data = (BsonData *) PG_GETARG_POINTER(0);
	text	   *field = PG_GETARG_TEXT_P(1);
	char	   *field_str = text_to_cstring(field);
