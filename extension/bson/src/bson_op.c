...
bson_eq_op(PG_FUNCTION_ARGS)
{
	...
	/* Simple binary comparison of BSON data */
	if (left->size != right->size)
	{
		PG_RETURN_BOOL(false);
	}

	result = (memcmp(left->data, right->data, left->size) == 0);
...
bson_type_op(PG_FUNCTION_ARGS)
{
	...
	/* Check if it's a wrapped standalone value */
	if (bson_iter_init_find(&iter, bson, "value"))
	{
		/* For wrapped standalone values, check the "value" field type */
	...
	else if (bson_iter_init(&iter, bson))
	{
		...
		{
			/* Empty document */
			result = (type_code == BSON_TYPE_DOCUMENT);
		}
		else
		{
			/* Check if the first field indicates array structure */
			const char *key = bson_iter_key(&iter);
			if (key && strcmp(key, "0") == 0)
			{
				/* Looks like an array (keys are "0", "1", etc.) */
				result = (type_code == BSON_TYPE_ARRAY);
			}
			else
			{
				/* Regular document */
