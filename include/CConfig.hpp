...
struct ConfigEntry
{
    ...
    ConfigEntry()
        : required(false), sensitive(false), source(ConfigSource::DEFAULT)
    {
    }
};
...
class CConfig
{
  ...
  private:
    ...
    std::unique_ptr<std::unordered_map<std::string, ConfigValue>>
        config_values_;
    std::unique_ptr<std::unordered_map<std::string, ConfigEntry>>
        config_metadata_;
    bool initialized_;
