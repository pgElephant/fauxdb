...
public:
	...
	void logWithContext(CLogLevel level, const std::string& message, const std::string& context);
	void logWithTimestamp(CLogLevel level, const std::string& message);
	void logWithMetadata(CLogLevel level, const std::string& message, const std::unordered_map<std::string, std::string>& metadata);
...
private:
	...
	std::string generateRotatedFileName(const std::string& baseName, size_t index);
};
