...
class CAuthRegistry
{
  public:
    ...
    std::shared_ptr<IAuthentication> getAuth(AuthType type, AuthDirection direction);
    ...
    std::shared_ptr<IAuthentication> createAuth(AuthType type, AuthDirection direction, const AuthConfig& config);
    std::shared_ptr<IPostgreSQLAuth> createPostgreSQLAuth(AuthType type, const AuthConfig& config);
    std::shared_ptr<IMongoDBAuth> createMongoDBAuth(AuthType type, const AuthConfig& config);
    ...
    bool loadFromConfig(const std::string& configKey, const std::string& configValue);
