...
struct PLibpqConfig
{
    ...
    PLibpqConfig()
        : host("localhost"), port("5432"), database(""), username(""),
          password(""), sslmode("prefer"), applicationName("FauxDB"),
          clientEncoding("UTF8"), timezone("UTC"), binaryResults(false),
          preparedStatements(true), connectionTimeout(5000),
          queryTimeout(30000), maxConnections(10), autoCommit(true),
          sslEnabled(false)
    {
    }
};
