...
struct PGConnection {
	...
	PGConnection(std::shared_ptr<CPostgresDatabase> db)
		: database(db), inUse(false), lastUsed(std::chrono::steady_clock::now()),
...
private:
	...
	void setConnectionValidator(function<bool(shared_ptr<void>)> validator) override;
	void setConnectionFactory(function<shared_ptr<void>()> factory) override;
	void setConnectionAcquiredCallback(function<void(shared_ptr<void>)> callback) override;
	void setConnectionReleasedCallback(function<void(shared_ptr<void>)> callback) override;
	void setConnectionFailedCallback(function<void(shared_ptr<void>, const string&)> callback) override;

protected:
	void logConnectionEvent(const string& event, const string& details) override;
