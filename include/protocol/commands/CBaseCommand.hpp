...
public:
    ...
    bool requiresDatabase() const override { return false; }

protected:
    /* Helper methods for BSON response creation */
    vector<uint8_t> createSuccessResponse(double okValue = 1.0);
    vector<uint8_t> createErrorResponse(int errorCode, const string& errorMessage);
    
    /* Helper methods for field extraction */
    string extractStringField(const vector<uint8_t>& buffer, ssize_t bufferSize, const string& fieldName);
    int32_t extractInt32Field(const vector<uint8_t>& buffer, ssize_t bufferSize, const string& fieldName);
    ...
    void addInferredType(CBsonType& bson, const string& fieldName, const string& value);
};
