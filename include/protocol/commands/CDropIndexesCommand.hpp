...
class CDropIndexesCommand : public CBaseCommand
{
  ...
  private:
    ...
    string extractIndexSpecifier(const vector<uint8_t>& buffer,
                                 ssize_t bufferSize);
    vector<string> getIndexesToDrop(const CommandContext& context,
                                    const string& collection,
                                    const string& specifier);
    bool dropIndex(const CommandContext& context, const string& collection,
                   const string& indexName);
    int32_t getIndexCount(const CommandContext& context,
                          const string& collection);
};
