...
CLogger::CLogger(const CServerConfig& config)
	: config_(config),
	  ...
	  consoleOutput_(true),
	  fileOutput_(true),
	  maxLogFileSize_(10485760),
	  logRotationCount_(5),
	  timestampFormat_("%Y-%m-%d %H:%M:%S"),
	  logLevel_(CLogLevel::INFO),
	  initialized_(false),
	  stopAsyncLogger_(false)
{
}
 ...
 */
void
CLogger::log(CLogLevel level, const std::string& message)
{
 ...
 */
void
CLogger::setLogLevel(CLogLevel level)
{
	logLevel_ = level;
}
 ...
 */
CLogLevel
CLogger::getLogLevel() const noexcept
{
	return logLevel_;
}
...
CLogger::initialize()
{
	...
	try
	{
		...
		{
			fileStream_ = std::make_unique<std::ofstream>(logFile_, std::ios::app);
		...
		{
			errorStream_ = std::make_unique<std::ofstream>(errorLogFile_, std::ios::app);
 ...
 */
void
CLogger::setLogFile(const std::string& filename)
{
	logFile_ = filename;
}
 ...
 */
void
CLogger::setErrorLogFile(const std::string& filename)
{
 ...
 */
void
CLogger::enableConsoleOutput(bool enable)
{
 ...
 */
void
CLogger::enableFileOutput(bool enable)
{
	fileOutput_ = enable;
}
 ...
 */
void
CLogger::setMaxLogFileSize(size_t maxSize)
{
 ...
 */
void
CLogger::setLogRotationCount(size_t count)
{
 ...
 */
void
CLogger::setTimestampFormat(const std::string& format)
{
 ...
 */
void
CLogger::writeToConsole(const std::string& message)
{
	if (consoleOutput_)
 ...
 */
void
CLogger::writeToFile(CLogLevel /* level */, const std::string& message)
{
	if (fileStream_ && fileStream_->is_open())
	{
 ...
 */
std::string
CLogger::formatMessage(CLogLevel level, const std::string& message)
{
	std::stringstream	ss;
	...
	std::string			component = config_.serverName.empty() ? "fauxdb" : config_.serverName;
 ...
 */
std::string
CLogger::getLevelString(CLogLevel level) const
{
	switch (level)
	{
 ...
 */
bool
CLogger::shouldLog(CLogLevel level) const noexcept
{
