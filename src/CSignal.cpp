...
{
	if (!initialized_ || !validateSignalType(signalType) || !isValidProcessId(processId))
	{
		return false;
	}
	...
	try
	{
		...
		{
			setError("Failed to send signal: " + std::string(std::strerror(errno)));
			return false;
		}
	}
	catch (const std::exception& e)
...
std::string getSignalDescription(int signalNumber)
{
	...
	case SIGALRM: return "Timer signal";
	case SIGCHLD: return "Child stopped or terminated";
	case SIGCONT: return "Continue if stopped";
	case SIGSTOP: return "Stop process";
	case SIGTSTP: return "Stop typed at terminal";
	case SIGTTIN: return "Terminal input for background process";
	case SIGTTOU: return "Terminal output for background process";
