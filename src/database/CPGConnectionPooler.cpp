...
shared_ptr<PGConnection> CPGConnectionPooler::getPostgresConnection()
{
    ...
    if (!availableConnections_.empty())
    {
        ...
        {
            availableConnections_.pop_back();  // Remove AFTER validation but
                                               // BEFORE markConnectionInUse
            inUseConnections_.push_back(conn); // Directly add to inUse list
            logConnectionEvent(
                "CONNECTION_ACQUIRED",
                "Connection acquired from pool. Ptr: " +
                    std::to_string(reinterpret_cast<uintptr_t>(conn.get())));
            return conn;
        }
        else
        {
...
void CPGConnectionPooler::releasePostgresConnection(
...
void CPGConnectionPooler::setConnectionFactory(
...
void CPGConnectionPooler::setConnectionTimeout(
...
void CPGConnectionPooler::setConnectionValidator(
    std::function<bool(std::shared_ptr<void>)> validator)
{
    std::lock_guard<std::mutex> lock(poolMutex_);
    connectionValidator_ = validator;
}

void CPGConnectionPooler::markConnectionAvailable(
    std::shared_ptr<PGConnection> connection)
{
    if (!connection)
    {
    ...
    auto it =
        std::find_if(inUseConnections_.begin(), inUseConnections_.end(),
    ...
    if (it != inUseConnections_.end())
    {
        ...
        logConnectionEvent(
            "CONNECTION_RELEASED",
            "Connection returned to pool. Ptr: " +
                std::to_string(reinterpret_cast<uintptr_t>(connection.get())));
    }
    else
    {
        logConnectionEvent(
            "CONNECTION_ERROR",
            "Attempted to release connection not in use. Ptr: " +
                std::to_string(reinterpret_cast<uintptr_t>(connection.get())) +
                ", InUse count: " + std::to_string(inUseConnections_.size()));
    }
}

void CPGConnectionPooler::setConnectionFailedCallback(
    std::function<void(std::shared_ptr<void>, const std::string&)> callback)
{
    std::lock_guard<std::mutex> lock(poolMutex_);
    connectionFailedCallback_ = callback;
}

void CPGConnectionPooler::setConnectionAcquiredCallback(
...
void CPGConnectionPooler::setConnectionReleasedCallback(
    std::function<void(std::shared_ptr<void>)> callback)
{
    std::lock_guard<std::mutex> lock(poolMutex_);
...
std::string CPGConnectionPooler::getStatusReport() const
{
    ...
    ss << "  Average Response Time: " << stats_.averageResponseTime.count()
       << "ms\n";
