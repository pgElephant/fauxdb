										...
										const std::vector<std::string>& params)
{
	...
	{
		result.success = false;
		result.message = "Exception during prepared query execution: " + std::string(e.what());
		return result;
	}
}
...
bool CPostgresDatabase::connect()
{
	...
	try
	{
		if (libpq_->connect(config_.host, config_.port, config_.database, config_.username, config_.password))
		{
...
std::string CPostgresDatabase::getServerVersion() const
{
	...
	try
	{
		// Note: This would need to be const-correct, but for now we'll use a workaround
		// In a real implementation, you'd need to make executeQuery const or have a separate const method
		return "PostgreSQL (version query not implemented)";
	}
	catch (const std::exception& e)
	{
		// Note: lastError_ is const in const methods, so we can't set it here
		// In a real implementation, you'd need a different approach for error handling in const methods
	}
