...
error_code CTcp::initialize()
{
    ...
    try
    {
        ...
        if (server_socket_ == -1)
        {
            std::cerr << "DEBUG: Failed to create socket, errno=" << errno
                      << " (" << strerror(errno) << ")" << std::endl;
            return make_error_code(errc::connection_refused);
        }
        std::cerr << "DEBUG: Socket created successfully, fd=" << server_socket_
                  << std::endl;
        int opt = 1;
        ...
        if (setsockopt(server_socket_, SOL_SOCKET, SO_REUSEADDR, &opt,
                       sizeof(opt)) < 0)
        {
            std::cerr << "DEBUG: Failed to set SO_REUSEADDR, errno=" << errno
                      << " (" << strerror(errno) << ")" << std::endl;
        ...
        initialized_ = true;
        std::cerr << "DEBUG: TCP server initialization completed successfully"
                  << std::endl;
        return error_code{};
    }
    catch (...)
...
error_code CTcp::start()
{
    ...
    try
    {
        std::cerr << "DEBUG: About to bind to address: " << config_.bindAddress
                  << ":" << config_.port << std::endl;
        auto bindResult = bindToAddress(config_.bindAddress, config_.port);
        if (bindResult)
        {
            std::cerr << "DEBUG: Bind failed: " << bindResult.message()
                      << std::endl;
            return bindResult;
        }
        ...
        {
            std::cerr << "DEBUG: Listen failed, errno=" << errno << " ("
                      << strerror(errno) << ")" << std::endl;
            return make_error_code(errc::connection_refused);
        }
...
void CTcp::connectionWorker(int clientSocket)
{
    ...
    while (running_.load())
    {
        ...
        auto response = documentHandler->processDocumentMessage(
            buffer, bytesRead, *responseBuilder);
        if (!response.empty())
