...
std::vector<uint8_t> CDocumentMsgSection1::serialize() const
{
    ...
    // Compute and write size: includes the int32 itself + identifier cstring +
    // docs
                                       ...
                                       size_t& offset)
{
    ...
    /* Read documents */
    size_t endOffset =
        sectionStartAfterSize + (size >= 4 ? static_cast<size_t>(size - 4) : 0);
...
CDocumentMsgBody& CDocumentMsgBody::operator=(const CDocumentMsgBody& other)
{
    if (this != &other)
    {
        ...
        {
            sections0.push_back(
                std::make_unique<CDocumentMsgSection0>(*section));
        }
        ...
        {
            sections1.push_back(
                std::make_unique<CDocumentMsgSection1>(*section));
        }
    }
    return *this;
}
...
void CDocumentMsgBody::addSection0(
    std::unique_ptr<CDocumentMsgSection0> section)
{
    sections0.push_back(std::move(section));
}

void CDocumentMsgBody::addSection1(
    std::unique_ptr<CDocumentMsgSection1> section)
{
    sections1.push_back(std::move(section));
}
...
{
    // Validation is performed at message level since checksum covers
    // header+body.
    return true;
}
...
void CDocumentWireMessage::setCompressedBody(
    const CDocumentCompressedBody& body)
{
...
bool CDocumentWireMessage::parseFromBytes(const std::vector<uint8_t>& data)
{
    ...
    case CDocumentOpCode::OP_MSG:
    {
        ...
        // If checksumPresent, validate CRC-32C over entire message minus last 4
        // bytes
        if (msgBody_->flagBits &
            static_cast<int32_t>(CDocumentMsgFlags::CHECKSUM_PRESENT))
        {
            if (header_.messageLength < 20 ||
                header_.messageLength > static_cast<int32_t>(data.size()))
            {
                return false;
            }
...
CDocumentWireMessage::createResponse() const
{
...
std::unique_ptr<CDocumentWireMessage>
CDocumentWireMessage::createDeleteResponse(int32_t requestID,
