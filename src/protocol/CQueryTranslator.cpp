...
CQueryTranslator::CQueryTranslator(std::shared_ptr<FauxDB::CDatabase> database)
...
std::string
CQueryTranslator::translateProjection(const std::string& docProjection)
{
    ...
    while (std::getline(iss, pair, ','))
    {
        ...
        if (!pair.empty())
        {
            ...
            if (colonPos != std::string::npos)
            {
                ...
                if (!field.empty())
                {
                    setClauses.push_back(escapeSQLIdentifier(field) + " = " +
...
std::string
CQueryTranslator::translateAggregation(const std::string& docAggregation)
{
...
std::string CQueryTranslator::translateIndex(const std::string& docIndex)
{
    ...
    sql += "idx_" + currentCollection_ + " ON " +
           buildTableName("", currentCollection_);
...
std::string
CQueryTranslator::translateTextSearch(const std::string& docTextSearch)
{
...
std::string CQueryTranslator::convertDocumentTypeToPostgreSQL(
