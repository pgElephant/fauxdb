...
string CollectionNameParser::extractCollectionName(
    const vector<uint8_t>& buffer, ssize_t bytesRead, const string& commandName)
{
    ...
    try
    {
        return parseBsonForCollection(buffer, 4,
                                      commandName); /* Skip document length */
    }
    catch (...)
    {
        return "";
...
string CollectionNameParser::parseBsonForCollection(
    const vector<uint8_t>& buffer, size_t offset, const string& commandName)
{
    while (offset < buffer.size())
    {
        ...
        if (type == 2 &&
            isCollectionField(fieldName, commandName)) /* String type */
        {
...
string CollectionNameParser::readBsonString(const vector<uint8_t>& buffer,
                                            size_t offset,
                                            uint32_t stringLength)
{
...
bool CollectionNameParser::isCollectionField(const string& fieldName,
                                             const string& commandName)
{
    ...
    return fieldName == "collection" || fieldName == "find" ||
           fieldName == "findOne" || fieldName == "countDocuments" ||
           fieldName == "count" || fieldName == "estimatedDocumentCount";
}
