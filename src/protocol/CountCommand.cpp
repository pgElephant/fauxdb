...
int64_t
CountCommand::executeCountQuery(const string& collection, shared_ptr<CPostgresDatabase> database)
{
    if (!database)
    {
        return 0;
...
vector<uint8_t>
EstimatedDocumentCountCommand::execute(
    const string& collection,
    const vector<uint8_t>& buffer,
    ssize_t bytesRead,
    shared_ptr<CPGConnectionPooler> connectionPooler)
