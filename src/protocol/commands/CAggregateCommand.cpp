...
string CAggregateCommand::getCommandName() const
{
    return "aggregate";
}

vector<uint8_t> CAggregateCommand::execute(const CommandContext& context)
{
...
bool CAggregateCommand::requiresDatabase() const
{
    return true;
}
...
CAggregateCommand::executeWithDatabase(const CommandContext& context)
{
    ...
    vector<PipelineStage> pipeline =
        extractPipeline(context.requestBuffer, context.requestSize);
    ...
    try
    {
        ...
        if (voidConnection)
        {
            ...
            if (result.success)
            {
                ...
                CBsonType cursor =
                    createCursorResponse(result.rows, result.columnNames);
...
CAggregateCommand::extractPipeline(const vector<uint8_t>& buffer,
                                   ssize_t bufferSize)
{
...
string
CAggregateCommand::convertPipelineToSQL(const vector<PipelineStage>& pipeline,
...
string CAggregateCommand::handleMatchStage(const string& operation)
{
...
string CAggregateCommand::handleGroupStage(const string& operation)
{
...
string CAggregateCommand::handleSortStage(const string& operation)
{
...
string CAggregateCommand::handleLimitStage(const string& operation)
{
...
string CAggregateCommand::handleSkipStage(const string& operation)
{
...
CBsonType
CAggregateCommand::createCursorResponse(const vector<vector<string>>& rows,
