...
string
CCollStatsCommand::getCommandName() const
{
    return "collStats";
}
...
vector<uint8_t>
CCollStatsCommand::execute(const CommandContext& context)
{
...
bool
CCollStatsCommand::requiresDatabase() const
{
    return true;
}
...
{
    /* PostgreSQL implementation for collStats */
    ...
    try
    {
        ...
        /* Build response with collection statistics */
...
CollectionStats
CCollStatsCommand::collectCollectionStats(const CommandContext& context, const string& collection)
{
    ...
    try
    {
        ...
        stats.totalIndexSize = stats.storageSize * 0.15; /* Estimate 15% for indexes */
        ...
        if (stats.scaleFactor != 1.0)
        {
            ...
            stats.totalIndexSize = (int64_t)(stats.totalIndexSize / stats.scaleFactor);
        }
    }
    catch (const std::exception& e)
    {
...
int64_t
CCollStatsCommand::getCollectionSize(const CommandContext& context, const string& collection)
{
    try
    {
...
int64_t
CCollStatsCommand::getCollectionCount(const CommandContext& context, const string& collection)
{
    try
    {
        auto voidConnection = context.connectionPooler->getConnection();
...
int64_t
CCollStatsCommand::getTableSize(const CommandContext& context, const string& collection)
{
    try
    {
        auto voidConnection = context.connectionPooler->getConnection();
        if (voidConnection)
        {
            ...
            string sql = "SELECT pg_total_relation_size('\"" + collection + "\"')";
            auto result = connection->database->executeQuery(sql);
...
double
CCollStatsCommand::extractScale(const vector<uint8_t>& buffer, ssize_t bufferSize)
{
...
CBsonType
CCollStatsCommand::createStatsResponse(const CollectionStats& stats)
{
    CBsonType response;
    response.initialize();
