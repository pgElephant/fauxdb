...
string
CCreateIndexesCommand::getCommandName() const
{
...
vector<uint8_t>
CCreateIndexesCommand::execute(const CommandContext& context)
{
...
bool
CCreateIndexesCommand::requiresDatabase() const
{
    return true;
}
...
CCreateIndexesCommand::executeWithDatabase(const CommandContext& context)
{
    ...
    vector<IndexSpec> indexSpecs = extractIndexSpecs(context.requestBuffer, context.requestSize);
...
CCreateIndexesCommand::executeWithoutDatabase(const CommandContext& context)
{
    ...
    bson.addInt32("numIndexesAfter", 1 + static_cast<int32_t>(indexSpecs.size()));
    bson.addBool("createdCollectionAutomatically", false);
    bson.addString("note", "indexes created on existing collection");
    bson.addDouble("ok", 1.0);
...
vector<IndexSpec>
CCreateIndexesCommand::extractIndexSpecs(const vector<uint8_t>& buffer, ssize_t bufferSize)
{
...
string
CCreateIndexesCommand::buildCreateIndexSQL(const string& collection, const IndexSpec& spec)
{
...
string
CCreateIndexesCommand::convertKeyPatternToSQL(const string& keyPattern)
{
...
string
CCreateIndexesCommand::generateIndexName(const string& collection, const IndexSpec& spec)
{
    if (!spec.name.empty())
    {
        return spec.name;
...
bool
CCreateIndexesCommand::indexExists(const CommandContext& context, const string& collection, const string& indexName)
{
    try
    {
        auto voidConnection = context.connectionPooler->getConnection();
        if (voidConnection)
        {
            ...
            string sql = "SELECT 1 FROM pg_indexes WHERE tablename = '" + collection + "' AND indexname = '" + indexName + "'";
            auto result = connection->database->executeQuery(sql);
