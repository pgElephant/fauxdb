...
string
CDistinctCommand::getCommandName() const
{
    return "distinct";
}
...
vector<uint8_t>
CDistinctCommand::execute(const CommandContext& context)
{
...
bool
CDistinctCommand::requiresDatabase() const
{
    return true;
}
...
CDistinctCommand::executeWithDatabase(const CommandContext& context)
{
    ...
    string fieldName = extractFieldName(context.requestBuffer, context.requestSize);
...
string
CDistinctCommand::extractFieldName(const vector<uint8_t>& buffer, ssize_t bufferSize)
{
