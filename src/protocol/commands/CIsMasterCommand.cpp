 ...
 *      IsMaster command implementation for document database (legacy compatibility)
 *
...
string
CIsMasterCommand::getCommandName() const
{
    return "isMaster";
}
...
vector<uint8_t>
CIsMasterCommand::execute(const CommandContext& context)
{
...
bool
CIsMasterCommand::requiresDatabase() const
{
...
CBsonType
CIsMasterCommand::createLegacyResponse()
{
    CBsonType response;
    response.initialize();
...
string
CIsMasterCommand::getServerHostname()
{
    char hostname[256];
...
int32_t
CIsMasterCommand::getServerPort()
{
