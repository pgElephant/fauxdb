...
string CListIndexesCommand::getCommandName() const
{
    return "listIndexes";
}

vector<uint8_t> CListIndexesCommand::execute(const CommandContext& context)
{
...
bool CListIndexesCommand::requiresDatabase() const
{
    return true;
}
...
vector<IndexInfo>
CListIndexesCommand::getCollectionIndexes(const CommandContext& context,
                                          const string& collection)
{
    ...
    try
    {
        ...
        if (voidConnection)
        {
            ...
            string sql = "SELECT indexname, indexdef FROM pg_indexes WHERE "
                         "tablename = '" +
                         collection + "'";
            ...
            if (result.success)
            {
                for (const auto& row : result.rows)
                {
                    if (row.size() >= 2)
                    {
                        ...
                        info.name = row[0];
                        info.keyPattern =
                            "{\"field\": 1}"; /* Simplified - would parse
                                                 indexdef */
                        info.version = 2;
...
CBsonType CListIndexesCommand::createIndexInfoDocument(const IndexInfo& info)
{
    CBsonType doc;
    doc.initialize();
    doc.beginDocument();
...
CBsonType
CListIndexesCommand::createCursorResponse(const vector<IndexInfo>& indexes,
                                          const string& ns)
{
    CBsonType cursor;
    cursor.initialize();
